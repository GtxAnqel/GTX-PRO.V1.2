...
function bilgi_goster() {
  ...
  data=$(curl -s "http://ip-api.com/json/$ip?fields=status,country,regionName,city,org,query,as")
  status=$(echo "$data" | jq -r '.status')

  if [[ "$status" != "success" ]]; then
    echo -e "${RED}Hata: IP bilgisi alınamadı.${RESET}"
    return
  fi

  echo -e "${GREEN}${BOLD}IP BİLGİSİ:${RESET}"
  echo -e "  IP     : $(echo "$data" | jq -r '.query')"
  echo -e "  Ülke   : $(echo "$data" | jq -r '.country')"
  echo -e "  Bölge  : $(echo "$data" | jq -r '.regionName')"
  echo -e "  Şehir  : $(echo "$data" | jq -r '.city')"
  echo -e "  ISP    : $(echo "$data" | jq -r '.org')"
  echo -e "  ASN    : $(echo "$data" | jq -r '.as')"
  echo ""
}
...
function usercan() {
  ...
  echo -e "\033[36mKullanıcı adı tarayıcı. Eğitim amaçlı.\033[0m"
...
function x_osint() {
  ...
  read -rp "Domain/IP: " hedef

  echo -e "${YELLOW}Whois Bilgisi:${RESET}"
  whois "$hedef" | head -20
...
function admin_finder() {
  read -rp "Hedef site (http://...): " site
  paneller=("admin" "admin/login" "admin.php" "admin/index.php" "panel" "cpanel")

  for yol in "${paneller[@]}"; do
    tam_url="${site}/${yol}"
    kod=$(curl -s -o /dev/null -w "%{http_code}" "$tam_url")
    if [[ "$kod" == "200" ]]; then
      echo -e "\033[32m[✓] Panel bulundu: $tam_url\033[0m"
    fi
  done
...
function exploit_scanner() {
  ...
  curl -s "https://www.exploit-db.com/search?description=$kelime" | grep -Eo '/exploits/[0-9]+' | head -5 | while read -r path; do
...
function menu() {
  while true; do
    ...
    echo "2) Usercan"
    echo "3) X-OSINT"
    echo "4) Admin Panel Finder"
    echo "5) Exploit Scanner"
    echo "6) Çıkış"
